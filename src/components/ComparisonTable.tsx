...
export default function ComparisonTable({ brokers, columns, cells }: Props) {
  const [activeColId, setActiveColId] = useState(columns[0]?.id ?? '')
              ...
              <ul className="mini-dropdown__panel">
                {columns.map((col) => (
                  <li key={col.id}>
                    <button
                      type="button"
                      className={`mini-dropdown__option${col.id === activeColId ? ' is-selected' : ''}`}
                      onClick={() => { setActiveColId(col.id); setIsOpen(false) }}
                    >
                      {col.label}
                    </button>
                  </li>
      ...
      {brokers.map((b) => (
        <div key={b.broker_id} className="bb-mini-table__row">
          ...
          <div className="bb-mini-table__cell">
            {renderCell(cells[`${b.broker_id}_${activeCol.id}`] ?? '')}
          </div>
        </div>
      ))}
    </div>
